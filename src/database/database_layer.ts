...
let database_layer = {
    ...
    db_read_single_record: async (collection: any, read_filter: Object | any ={}, projection_filter: Object | any ={}): Promise<any> => {
        return await collection.findOne(read_filter).projection(projection_filter);
    },
    db_read_multiple_records: async (collection: any, read_filter: Object | any ={}, projection_filter: Object | any ={}, pageOptions: Object | any = {}, is_deleted: Boolean =false) => {
        if (!is_deleted) {
            read_filter[constants.STATUS] = { id: 1, name: "active" }
        }
        if ( Object.keys(pageOptions).length != 0 ) {
            return await collection.find(read_filter).skip( (pageOptions.page - 1) * pageOptions.limit ).limit(pageOptions.limit).sort({ created_at: -1 }).projection(projection_filter);
        }
        else {
