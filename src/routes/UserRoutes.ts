...
const Multer: Multer = multer({
    ...
    limits: {fileSize: 5 * 1024 * 1024}, // These limits mention 5MB max upload limit for multer
});
...
router.put('/update', authentication_middleware, async (req: Request, res: Response) => {
    let required_list: Array<string> = [constants.ID];
...
router.post('/reset-password', async (req: Request, res: Response) => {
    let required_list: Array<string> = [constants.ID, constants.TOKEN, constants.NEW_PASSWORD];
    let response = await commonUtils.validate_request_body(req.body, required_list, []);
    if (response["statusCode"] != 200) {
        logger.error(`Missing Paramters: ${JSON.stringify(response["message"])}`);
        return res.status(200).send(response);
...
router.post('/change-password', authentication_middleware, async (req: Request, res: Response) => {
    let required_list: Array<string> = [constants.ID, constants.OLD_PASSWORD, constants.NEW_PASSWORD];
    let response = await commonUtils.validate_request_body(req.body, required_list, []);
    if (response["statusCode"] != 200) {
        logger.error(`Missing Paramters: ${JSON.stringify(response["message"])}`);
        return res.status(200).send(response);
